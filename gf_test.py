...
x_b = np.hstack([x, np.ones((len(x), 1))]);x_b
...
A = 2./len(y)*X.T.dot(X) # ŋ=1
J = lambda w: np.mean((X.dot(w)-y)**2)
gJ = lambda w: 2./len(y)*X.T.dot(X.dot(w)-y)
#A = X.T@X # ŋ=1/n
...
#%%
iters_gd_plot(rets,var,pgrid,paras=skwargs,
              **kwargs)